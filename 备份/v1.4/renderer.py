...
def make_blockers(_renderer):
    ...
    for _object in objects:
        ...
        if properties['name'] == 'blocker':
            ...
            blocker = Object(pygame.Rect(left, top, width, height), properties['type'])
            _blockers.add(blocker)
    return _blockers


def make_door(_renderer):
    _door = pygame.sprite.Group()
    objects = _renderer.tmx_data.get_layer_by_name("object")
    for _object in objects:
        properties = _object.__dict__
        if properties['name'] == 'door':
            ...
            width = properties['width']
            height = properties['height']
            door = Object(pygame.Rect(left, top, width, height), properties['type'])
            _door.add(door)
    return _door